...
function save_transaction($content, $approver) {
	global $wpdb;
	$query = 'INSERT INTO sy_transactions (dd_amount, dd_bank, dd_number, dd_date, pay_status, cash_amount, remarks, approver ) VALUES (%f, %s, %s, %s, %d, %f, %s, %d)';
	$wpdb->query($wpdb->prepare($query, $content['dd_amount'], $content['dd_bank'], $content['dd_number'], $content['dd_date'], $content['pay_status'], $content['cash_amount'], $content['remarks'], $approver));
	$transaction_id = $wpdb->insert_id;
	return $transaction_id;
}
function update_transaction($content, $approver) {
	global $wpdb;
	$query = 'UPDATE sy_transactions SET dd_amount = %f, dd_bank = %s, dd_number = %s, dd_date = %s, pay_status = %d, cash_amount = %f, remarks = %s, approver = %d WHERE id = %d';
	$wpdb->query($wpdb->prepare($query, $content['dd_amount'], $content['dd_bank'], $content['dd_number'], $content['dd_date'], $content['pay_status'], $content['cash_amount'], $content['remarks'], $approver, $content['transaction_id']));
	return 1;
}
...
function get_transaction($transaction_id) {
	global $wpdb;
	$query = 'SELECT dd_amount, dd_bank, dd_number, dd_date, pay_status, cash_amount, remarks, approver FROM sy_transactions WHERE id = '.$transaction_id;
	$rs = $wpdb->get_row($query);
	return $rs;
}
...
function get_registrations($params){
	...
	if($params['q_type'] == 'fill') {
		...
		$query = 'SELECT ep.id, p.first_name, p.last_name, p.contact_no, se.name as subevent, se.start_date, se.end_date, se.event_type, t.pay_status, ep.card_issued, ep.reg_centre, ep.registered_date, ep.reg_number FROM sy_event_participation ep INNER JOIN sy_participants p ON ep.participant_id = p.id LEFT JOIN sy_transactions t ON ep.transaction_id = t.id INNER JOIN sy_subevent se on ep.subevent_id = se.id '.$where.' ORDER BY ep.id DESC '.$limit_clause;
	}
	$rs = $wpdb->get_results($query);
	return $rs;
}